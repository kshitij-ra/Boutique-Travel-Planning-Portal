  ...
  return (
    <div className={`ppb2-ppb2 ${props.rootClassName} `}>
      ...
      <div className="ppb2-container">
        ...
        {tags.map(tag => (
          <div className="ppb2-checkbox-container">
            <input type="checkbox" value={tag} checked={selectedTags.includes(tag)} onChange={() => handleTagSelection(tag)} className="ppb2-checkbox" />
            <span className="">{tag}</span>
          </div>
          ))}
        </div>
        <div className="ppb2-container2">
          <OutlineButton
            button1="Previous"
            rootClassName="outline-button-root-class-name5"
            className=""
            onClick={props.prev}
          ></OutlineButton>
          <SolidButton
            rootClassName="solid-button-root-class-name13"
            button="Continue"
            //onclick - fetch destintion based on selected tags using api 
            onClick={() => {console.log('Selected Tags:', selectedTags);{props.next}}}
            className=""
          ></SolidButton>
        </div>
      </div>
